...
export class ShareBundleService {
    sodium = null;
    async ensureSodium() {
        if (!this.sodium) {
    ...
    async createBundle(config) {
        try {
            ...
            throw new SecurityError('Failed to create share bundle', SecurityErrorCode.ENCRYPTION_FAILED, error);
        }
    }
    /**
     * Extract share bundle
     */
    async extractBundle(bundle, password) {
        try {
            await this.ensureSodium();
            if (bundle.length <= 32) {
            ...
            // Parse JSON
            const bundleData = JSON.parse(decrypted.data);
            // Verify expiration
            ...
            throw new SecurityError('Failed to extract share bundle - invalid password or corrupted data', SecurityErrorCode.DECRYPTION_FAILED, error);
        }
    }
    /**
     ...
     */
    async getBundleMetadata(bundle) {
        try {
            ...
            throw new SecurityError('Failed to read bundle metadata', SecurityErrorCode.INVALID_BUNDLE, error);
        }
    }
    /**
     ...
     */
    async createBundleWithSodium(config) {
        try {
            ...
            const key = ensureBuffer(await sodium.crypto_pwhash(32, config.password, salt, 2, // opsLimit (minimum)
            64 * 1024 * 1024, // memLimit (64 MB)
            sodium.CRYPTO_PWHASH_ALG_ARGON2ID13));
            // Encrypt
            const nonce = ensureBuffer(await sodium.randombytes_buf(24));
            const ciphertext = await sodium.crypto_secretbox(dataBuffer, nonce, new CryptographyKey(ensureBuffer(key)));
            ...
            throw new SecurityError('Failed to create bundle with libsodium', SecurityErrorCode.ENCRYPTION_FAILED, error);
        }
    }
    /**
     * Extract bundle created with libsodium
     */
    async extractBundleWithSodium(bundle, password) {
        try {
            const sodium = await this.ensureSodium();
            // Parse bundle format
            ...
            // Derive key from password
            const key = ensureBuffer(await sodium.crypto_pwhash(32, password, salt, 2, 64 * 1024 * 1024, sodium.CRYPTO_PWHASH_ALG_ARGON2ID13));
            // Decrypt
            const plaintext = await sodium.crypto_secretbox_open(ciphertext, nonce, new CryptographyKey(ensureBuffer(key)));
            // Parse data
            const bundleData = JSON.parse(plaintext.toString('utf-8'));
            return {
                notes: bundleData.notes,
                attachments: bundleData.attachments,
                metadata: bundleData.metadata,
            ...
            throw new SecurityError('Failed to extract bundle - invalid password or corrupted data', SecurityErrorCode.DECRYPTION_FAILED, error);
        }
    }
    /**
     * Get metadata from libsodium bundle (without password)
     */
    async getSodiumBundleMetadata(bundle) {
        try {
