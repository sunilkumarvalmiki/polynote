...
import type { ISessionStorage, UserSession } from './types.js';
import { AuthError, AuthErrorCode } from './types.js';
import type { IEncryptionService } from '../types/index.js';

/**
 * Session Manager implementation with encrypted storage
 */
export class SessionManager implements ISessionStorage {
  ...
  async saveSession(session: UserSession): Promise<void> {
    try {
      ...
      throw new AuthError(
        `Failed to save session for user ${session.userId}`,
  ...
  async updateSession(userId: string, updates: Partial<UserSession>): Promise<void> {
    try {
      ...
      if (!existingSession) {
        throw new AuthError(
          `Session not found for user ${userId}`,
          AuthErrorCode.SESSION_NOT_FOUND
        );
      }
      ...
      throw new AuthError(
        `Failed to update session for user ${userId}`,
        AuthErrorCode.OAUTH_FAILED,
        error as Error
      );
    }
  ...
  async getAllActiveSessions(): Promise<UserSession[]> {
    try {
      ...
      throw new AuthError(
        'Failed to get all active sessions',
        AuthErrorCode.OAUTH_FAILED,
        error as Error
      );
    }
