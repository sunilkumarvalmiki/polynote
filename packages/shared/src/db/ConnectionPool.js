...
export class ConnectionPool extends EventEmitter {
    ...
    async readOne(sql, params) {
        const results = await this.read(sql, params);
    ...
    cleanupIdleConnections() {
        ...
        if (this.readConnections.length <= 1)
            return;
        const toRemove = this.readConnections.filter(c => !c.inUse && c.lastUsed < threshold);
